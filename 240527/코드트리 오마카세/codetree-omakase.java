...
public class Main {
    ...
    static boolean cmp(Query q1, Query q2) {
        if (q1.t != q2.t)
            return q1.t < q2.t;
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < Q; i++) {
            ...
            Query query = new Query(cmd, t, x, name, n);
            queries.add(query);
        ...
        for (Query query : queries) {
            if (query.cmd == 100) // 초밥 추가
                sushi_num++;
            else if (query.cmd == 111) // 초밥 제거
                sushi_num--;
            else if (query.cmd == 200) // 사람 추가
                people_num++;
            else if (query.cmd == 222) // 사람 제거
                people_num--;
