...
public class Main {
    ...
    public static int R, C, K; // 행, 열, 골렘의 개수를 의미합니다
    public static int[][] A = new int[MAX_L+3][MAX_L]; // 실제 숲을 [3~R+2][0~C-1]로 사용하기위해 행은 3만큼의 크기를 더 갖습니다
    public static int[] dy = {-1, 0, 1, 0}, dx = {0, 1, 0, -1};
    public static boolean[][] isExit = new boolean[MAX_L+3][MAX_L]; // 해당 칸이 골렘의 출구인지 저장합니다
    public static int answer; // 각 정령들이 도달할 수 있는 최하단 행의 총합을 저장합니다
    ...
    public static void main(String[] args) {
        ...
        for (int id = 1; id <= K; id++) { // 골렘 번호 id
            int x = sc.nextInt();
            int d = sc.nextInt(); // 골렘의 출발 x좌표, 방향 d를 입력받습니다
            x--;
            down(0, x, d, id);
        }
