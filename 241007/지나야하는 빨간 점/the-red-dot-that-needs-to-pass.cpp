...
// 각 좌표까지의 최소 빨간 점 통과 횟수를 저장하는 배열입니다.
int distance[1002][1002];

// 현재 위치에서 새로운 위치로 이동할 때의 거리를 갱신하는 함수입니다.
void updateDistance(int current_x, int current_y, int new_x, int new_y) {
    ...
    if (new_x >= 0 && new_x <= 1001 && new_y >= 0 && new_y <= 1001) {
        // 새로운 좌표의 거리가 아직 초기값이거나, 현재 경로를 통해 더 적은 빨간 점을 지나갈 수 있다면 갱신합니다.
        if (distance[new_x][new_y] == 0 || distance[current_x][current_y] + red_points[new_x][new_y] < distance[new_x][new_y]) {
            distance[new_x][new_y] = distance[current_x][current_y] + red_points[new_x][new_y];
            // 빨간 점을 지나지 않는 경우 zero_distance_queue에 추가합니다.
            if (red_points[new_x][new_y] == 0) {
                zero_distance_queue.push(Point(new_x, new_y));
            } else {
                // 빨간 점을 지나는 경우 one_distance_queue에 추가합니다.
...
int main() {
    ...
    while (!zero_distance_queue.empty() || !one_distance_queue.empty()) {
        // zero_distance_queue가 비어있으면 one_distance_queue의 내용을 zero_distance_queue로 옮깁니다.
